...
\begin{abstract}
  ...
  agents in a financial society. To simulate these transactions, we use Monte Carlo methods. Specifically, we will look at how to reduce wealth inequality with these models, and we will see that having small transactions, having no preference for the wealth of transaction partners, but high preference for previous partners, will do most to decrease wealth inequality.
\end{abstract}
...
The variance is the average squared difference between the agents' actual wealth and their average/initial money, $m_0$. It doesn't tell if there is extreme poverty, it tells us how much wealth inequality there is.
To calculate it, we simply iterate over all our agents' money:
\begin{equation}
...
We only calculate the variance for our first model, but we keep track of the variance at every step by tracking how $\expval{m^2}$ changes between each transaction. (See \cref{sec:result-model1}).
...
\subsection{First model: No preference} \label{sec:result-model1}
...
In fact, when we run the simulations, we get the distribution shown in \cref{fig:a}, and we see that the distribution matches almost exactly up with the expected distribution, where $\beta = m_0^{-1} = 1$. This is further confirmed by plotting the $y$ axis logarithmically, in which case we get \cref{fig:b}, which clearly shows this relation as a straight line.

We know that the variance of the Gibbs distribution is $\sigma_m^2 = \beta^{-2} = m_0^2$, and we can use this to find out how many transactions we need to get to equilibrium. Before any transactions, the variance is zero (as every agent has the same amount of money); after equilibrium is reached, we expect the variance to vary around the theoretical variance.
In \cref{fig:a-equilibrium} we see the calculated variance of the simulated distribution plotted against the number of transactions, for different numbers of agents. We can see that it takes only about \numrange{e2}{e3} transactions to reach equilibrium, and that this number is roughly proportional to $N$.
...
% If the distribution followed a tail power law, we would expect the plots in \cref{fig:d-log} to make a straight line in the tail. Instead\footnote{Awkward silence.}
...
In \cref{fig:e-nosave,fig:e-save,fig:e-nosave-log,fig:e-save-log}, we have again plotted the distributions of these simulations with linear and logarithmic axes.
By looking at these figures, we see that increasing the $\gamma$ parameter ``evens out'' the distribution again. If we compare this to the distributions of differing $\alpha$ values in \cref{fig:d-N1000}, we see that $\gamma$ parameters in effect ``negate'' the effects of the $\alpha$ parameter. By this, we mean that increasing $\gamma$ values causes wealth equality while increasing $\alpha$ values causes wealth inequality.

If we compare the figures without saving (\cref{fig:e-nosave-log}) with figures~5~and~6 of \textcite{goswami-sen}, we see that for $\alpha = 1.0$ the plots have a similar shape, while for $\alpha = 2.0$ the plots are drastically different. However, in both cases we still don't reproduce the kink that Goswami and Sen have in their figures.

For $m$ between approximately 2 and 20, we see that the distribution of $\alpha = 2.0$ stays approximately uniform.
...
\section{Conclusion}
Our models may be questionable for a realistic point of view, but from these models we get a sense of how the different effects affect the distribution of wealth in society. High saving factors $\lambda$ cause less risk-taking and smaller transactions, which gives us a narrower distribution and less wealth inequality. A high preferential factor $\alpha$, on the other hand, causes more people to be poor and rich people to be wealthier, which means that there is a higher wealth inequality. Finally, a high memory factor $\gamma$ means that the agents end up almost only trading with each other in small groups: this causes a limited flow of money and reduces the wealth inequality for most people.

Based on these models, we can tell what the ideal financial society should be if agents start with an equal amount of money: agents should trade only small amounts of money at a time; they should trade with no regards to the wealth oftheir transactional partners; and they should have a high preference for previous transactional partners. This combination will reduce income equality the most.
